...
pub fn print_help(config: &Config) -> io::Result<()> {
...
pub fn execute(config: &Config) -> RunStatus {
    let stdout = io::stdout();
...
pub fn describe_error(path: &str, err: &io::Error) -> String {
    match err.kind() {
...
pub fn choose_color_mode(config: &Config) -> ColorMode {
...
impl<'a> Printer<'a> {
    pub fn new(cfg: &'a Config, use_color: bool, color_mode: ColorMode, offset: f64) -> Self {
...
pub fn encode_component(value: f64) -> u8 {
