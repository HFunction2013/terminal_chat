...
#[register_hook]
fn set_global_option(option: &GlobalOption) -> GlobalOption {
...
#[register_hook]
fn exists_global_option(key: &safer_ffi::String) -> bool {
    let map = MAP.lock().unwrap();
    map.contains_key(&**key)
}

#[register_hook(fallback = "TaggedOption::None")]
fn get_global_option(key: &safer_ffi::String) -> TaggedOption<safer_ffi::String> {
    let map = MAP.lock().unwrap();
    map.get(&**key).cloned().map(|v| v.into()).into()
}

#[register_hook(fallback = "TaggedOption::None")]
fn remove_global_option(key: &safer_ffi::String) -> TaggedOption<safer_ffi::String> {
    let mut map = MAP.lock().unwrap();
    map.remove(&**key).map(|v| v.into()).into()
}

#[register_hook(fallback = "safer_ffi::vec::Vec::from(Vec::new())")]
fn get_all_options() -> safer_ffi::vec::Vec<GlobalOption> {
    let map = MAP.lock().unwrap();
    map.iter().map(|(k, v)| GlobalOption::new(k, v)).collect::<Vec<_>>().into()
}

#[register_hook]
fn clear_all_options() {
    let mut map = MAP.lock().unwrap();
    map.clear();
}
